...
test('empty or null past request', () => {
    ...
    expect(valid?.error).toBe('It is necessary to inform something in the request');
});

test('do not inform the valueA', () => {
    const request = { 'valueB': 10, 'valueC': 5 }
    const valid = BhaskaraHandler(request);
    expect(valid?.error).toBe('It is mandatory to inform the valueA and valueB and valueC');
});
